...
t.to_csv('../datda/all_pred.csv',index=None)




#根据补多、补少成本进行模型融合
t = pd.read_csv('../datda/all_pred.csv')
col_num = t.shape[1]
...
val_y_pred.to_csv('../data/estimate.csv',index=None)
final_pred = val_y_pred.iloc[:,[0,1,5]] 
val_y_pred.to_csv('../data/final_estimate.csv',index=None)
